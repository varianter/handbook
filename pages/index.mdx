...
Verdiene våre ligger til grunn for hvordan vi behandler hverandre. Det skal være lav terskel for ros og tilbakemeldinger fordi vi ønsker at du lykkes. Vær den som sier hva du har på hjertet ditt og som tør å utfordre sannheter og gjeldende praksis. [Konkrete tiltak for åpen selskapskultur i Variant](variant.no)

`Punktene nedenfor er aside elements (google it) som blobifiseres`
...
Det å være en variant er på ingen måte en statisk øvelse. Vi må alle utvikle oss for å alltid være godt egnet til å løse krevende oppgaver for våre kunder.
...
Dette er vårt oppstartsprogram og er skreddersydd for de som kommer rett fra skolen eller har mindre enn to års erfaring.

Merk, det er ingen tvil om at den viktigste læringen skjer i prosjekter ute hos kunder 37,5 timer i uka. For å forsterke læringen i prosjekt samt sikre at alle har et felles begrepsapparata og ett felles grunnlag har vi etablert variant:skudd
...
viser. [Les mer om hvert av stegene her](Expand link med chevron) 
...
`Stor tekst`Som konsulenter sitter vi ofte ute hos kunde, og derfor gjør vi en ekstra innsats for å finne på hyggelige ting sammen. 
...
`Stor tekst`Vi betaler godt, samtidig handler raushet om mye mer enn bare lønna.
...
<!-- Bygg inn lønnsvekst kalkulator / graf -->

Hva den enkelte tjener er ingen hemmelighet i Variant. Lurer du på hva du ville tjent hos oss? Vår hjemmesnekra [kalkulator](https://www.variant.no/kalkulator) 🧮 gir deg svaret!
...
<!-- Eksempel tekst får en bakgrunn i fargen #secondary4/L2 -->
...
<!-- Bygg inn kalkulator for pensjon -->

### Goder og ytelser

Vi kjører punktliste!

<!-- Punktlisten får en bakgrunn i fargen #secondary4/L2. Samme som eksempel under bonus --> 


`Ikon` Gadgetbudsjett på 10 000 pluss moms pr. år. Hva andre har brukt det til finner
  du på [oversikten her](https://dash.variant.no/gadgetusage/trondheim).
`Ikon` Ett kurs eller en konferanse i året
`Ikon` Fri bruk av telefoni – innen rimelighetens grenser.
`Ikon` Bredbånd hjemme på inntil 500 kr. per måned.
`Ikon` Betalt to uker permisjon for partner etter fødsel.
`Ikon` Full lønn under foreldrepermisjon, selv om lønnen skulle overstige det NAV betaler.
`Ikon` To dager til barnehagetilvenning.
`Ikon` Utvidede rettigheter til
  [egenmelding og sykepenger](information.html#sykemelding-og-egenmelding).
`Ikon` Dekt treningssentermedlemsskap, eller tilsvarende helsebringende
...
`Ikon` Behandlingsforsikring.
`Ikon` Reiseforsikring for deg og din familie.
